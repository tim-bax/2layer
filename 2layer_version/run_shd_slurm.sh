#!/bin/bash
#SBATCH -J SHD_2layer_JAX
#SBATCH -t 16:00:00
#SBATCH -o out/SHD_2layer_JAX_%j.out
...
# ---- Project root (parent of 2layer_version; data/ and 2layer_version/ live here) ----
PROJECT_DIR=/share/neurocomputation/Tim/git/2layer
mkdir -p "$PROJECT_DIR/2layer_version/out"
...
# ---- 2-layer SHD: overridable via env (edit here or export before sbatch) ----
export T_MS="${T_MS:-700}"
export N_EXTRA="${N_EXTRA:-64}"
export N_HIDDEN="${N_HIDDEN:-64}"
export N_OUTPUTS="${N_OUTPUTS:-20}"
export RANDOM_SEED="${RANDOM_SEED:-12}"
export EPOCHS="${EPOCHS:-100}"
export BATCH_SIZE="${BATCH_SIZE:-1}"
export LR_EXTRA_DEND="${LR_EXTRA_DEND:-0.05}"
export LR_EXTRA_SOMA="${LR_EXTRA_SOMA:-0.0025}"
export LR_HIDDEN_DEND="${LR_HIDDEN_DEND:-0.05}"
export LR_HIDDEN_SOMA="${LR_HIDDEN_SOMA:-0.0025}"
...
echo "N_EXTRA=$N_EXTRA  N_HIDDEN=$N_HIDDEN  NO_KERNEL=$NO_KERNEL  SPIKE_AMPLITUDE=$SPIKE_AMPLITUDE"
...
python 2layer_version/run_shd.py \
  --T "$T_MS" \
  --n_extra "$N_EXTRA" \
  --n_hidden "$N_HIDDEN" \
  --n_outputs "$N_OUTPUTS" \
  ...
  --lr_extra_dend "$LR_EXTRA_DEND" \
  --lr_extra_soma "$LR_EXTRA_SOMA" \
