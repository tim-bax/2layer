...
class JAXEPropNetworkTwoLayer:
    ...
    def _compute_gradients(self, params: NetworkParamsTwoLayer, x_input: jnp.ndarray, target: int,
        ...
        # Accumulate over time to avoid OOM: dmu_extra_at_tprime_h (T, n_hidden, n_extra, n_inputs) is huge for 64x64
        def _accumulate_dend_p2(carry, t):
            grad_acc = carry
            dmu_t = dmu_dw_extra[t_prime_h_int[t]]  # (n_hidden, n_extra, n_inputs)
            inc = (self.config.gamma / self.T) * jnp.einsum('ik,ikj->kj', term_d2[t], dmu_t)
            return grad_acc + inc, None
        grad_extra_dend_p2, _ = lax.scan(_accumulate_dend_p2, jnp.zeros((n_extra, n_inputs)), jnp.arange(T))
