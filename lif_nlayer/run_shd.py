#!/usr/bin/env python3
"""Multi-layer LIF e-prop on SHD (standalone lif_nlayer folder)."""
import argparse
import os
import sys
import time
...
def parse_args():
    p = argparse.ArgumentParser(description="Multi-layer LIF e-prop on SHD")
    ...
    p.add_argument(
        ...
        type=float,
        default=0.0,
        help="Dropout on spikes between hidden layers and into readout.",
    )
...
def main():
    ...
    print(
        f"Network: {n_inputs} -> {hid_str} (LIF) -> {args.n_classes} (LIF readout)  "
        f"optimizer={opt_str}  lr={args.lr}  dropout_hidden={args.dropout_hidden}",
        flush=True,
    )
