...
from one_neuron import NeuronConfig
from training import evaluate, train


def parse_args():
    p = argparse.ArgumentParser(description="Minimal modular 1-layer SHD run")
    ...
    p.add_argument("--lr_hidden_dend", type=float, default=0.045)
    p.add_argument("--lr_hidden_soma", type=float, default=0.00015)
    p.add_argument("--lr_readout", type=float, default=0.035)
    p.add_argument("--grad_dend_scale", type=float, default=1.0)
    ...
    p.add_argument("--warmup_readout_epochs", type=int, default=1)
    p.add_argument("--no_kernel", action="store_true")
    p.add_argument("--spike_amplitude", type=float, default=1.0)
    p.add_argument("--full_history", action="store_true", help="Use full-history gradient path instead of low-memory")
    return p.parse_args()
...
def main():
    ...
    print(f"Gradient mode: {'low-memory' if use_low_memory else 'full-history'}", flush=True)
    model = OneLayerEProp(
        key=key,
        n_inputs=n_inputs,
        ...
        grad_dend_scale=args.grad_dend_scale,
    ...
    history = train(
        ...
        warmup_readout_epochs=args.warmup_readout_epochs,
    )
